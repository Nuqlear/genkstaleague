...
class Season(db.Model):
    ...
    def end(self, date=None):
        ...
        self.ended = date
        stats = SeasonStats.query.filter(and_(SeasonStats.season_id == self.id,
                                                  (
                                                      SeasonStats.wins + SeasonStats.losses) > current_app.config.get(
                                                      'SEASON_CALIBRATING_MATCHES_NUM', 0))) \
            .with_entities(SeasonStats.steam_id, SeasonStats.pts).order_by(desc(SeasonStats.pts)).limit(
            3).all()
        for s in stats:
...
class SeasonStats(db.Model):
    ...
    def get_or_create(steam_id, season_id):
        s = SeasonStats.query.filter(
            and_(SeasonStats.season_id == season_id, SeasonStats.steam_id == steam_id)).all()
        if not s:
