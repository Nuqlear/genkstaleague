...
class Season(db.Model):
    ...
    place_1 = Column(BigInteger, ForeignKey('player.steam_id',
                                            onupdate="CASCADE",
                                            ondelete="CASCADE"))
    place_2 = Column(BigInteger, ForeignKey('player.steam_id',
                                            onupdate="CASCADE",
                                            ondelete="CASCADE"))
    place_3 = Column(BigInteger, ForeignKey('player.steam_id',
                                            onupdate="CASCADE",
                                            ondelete="CASCADE"))
    place_1_player = db.relationship('Player',
                                     foreign_keys='Season.place_1',
                                     backref=db.backref('place_1_seasons', lazy='dynamic'))
    place_2_player = db.relationship('Player',
                                     foreign_keys='Season.place_2',
                                     backref=db.backref('place_2_seasons', lazy='dynamic'))
    place_3_player = db.relationship('Player',
    ...
    def end(self, date=None):
        ...
        self.ended = date
        stats = (SeasonStats.query
                 .filter(and_(SeasonStats.season_id == self.id,
...
class SeasonStats(db.Model):
    ...
    steam_id = Column(BigInteger, ForeignKey('player.steam_id',
                                             onupdate="CASCADE",
                                             ondelete="CASCADE"), nullable=False)
    ...
    def get_or_create(steam_id, season_id):
        season = SeasonStats.query.filter(and_(SeasonStats.season_id == season_id, 
                                          SeasonStats.steam_id == steam_id)).all()
        if not season:
    ...
    def get_stats(season_number=-1, nickname_filter=None, sort='pts'):
        ...
        from gleague.frontend.seasons import get_season_id
        ...
        rv = (
            SeasonStats.query
                ...
                .having(func.count(PlayerMatchStats.id) > 
                        current_app.config['SEASON_CALIBRATING_MATCHES_NUM'])
        )
        if nickname_filter:
