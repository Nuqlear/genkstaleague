...
def records(season_number=-1):
    ...
    if longest_match:
        ...
        most_towerdamage_pms = db.session.query(PlayerMatchStats).join(SeasonStats) \
            .filter(and_(SeasonStats.season_id == s_id,
        ...
        most_damagetaken_pms = db.session.query(PlayerMatchStats).join(SeasonStats) \
            .filter(and_(SeasonStats.season_id == s_id,
        ...
        radiant_winrate = (100 * func.sum(case([(Match.radiant_win == True, 1)], else_=0)) /
                           func.count(Match.id))
        dire_winrate = (100 * func.sum(case([(Match.radiant_win == False, 1)], else_=0)) /
                        func.count(Match.id))
        side_winrates = (Match.query.join(Season)
                         .filter(Season.id == s_id)
