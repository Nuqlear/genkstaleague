...
class Season(db.Model):
    ...
    matches = relationship(
        'Match', lazy='dynamic', backref='season'
    )
    place_1 = Column(
        BigInteger,
        ForeignKey(
            'player.steam_id', onupdate="CASCADE", ondelete="CASCADE"
        )
    )
    place_2 = Column(
        BigInteger,
...
class SeasonStats(db.Model):
    ...
    def get_stats(season_number=-1, nickname_filter=None, sort='pts'):
        ...
        if nickname_filter:
            result = result.filter(
                func.lower(Player.nickname)
                .startswith(func.lower(nickname_filter))
            )
        return result
    ...
    def calibrated(self):
        from gleague.models import PlayerMatchStats
        return (
            db.session.query(func.count(PlayerMatchStats.id)).join(SeasonStats)
            .group_by(SeasonStats.id).filter(SeasonStats.id == self.id)
